...
// Helper function to determine preferred locale
fn get_preferred_locale(
    ...
    match user_preferred_language {
        ...
        None => {
            // Use Accept-Language header to determine preferred locale
...
pub async fn get_collaboration_meta(
    ...
    // Get session metadata - anyone with the link can access
    ...
    // Count distinct active users in the session
...
pub async fn save_collaborative_session(
    ...
    // Verify user is the session owner and session hasn't been saved yet
    let session = sqlx::query!(
        r#"
    ...
    let (post_id, owner_login_name) =
        handle_save_request(db, session_uuid, user.id, png_data, state)
            .await
...
async fn handle_socket(
    ...
    .await
    {
        Ok(Some(session)) => {
            ...
            // If user is not already a participant, check user limit
            ...
            // Update session activity in memory cache
    ...
    while let Some(msg) = receiver.next().await {
                            ...
                            if data.len() >= 25 {
                                if let Ok(user_uuid) = bytes_to_uuid(&data[1..17]) {
                                    ...
                                    if user_uuid == user_id {
                                        ...
                                        // Reconstruct JOIN message with login name
                                        // New format: [0x01][UUID:16][timestamp:8][usernameLength:2][username:variable]
                                        let username_bytes = user_login_name.as_bytes();
                                        let username_len = username_bytes.len() as u16;

                                        let mut new_join_msg = Vec::new();
                                        new_join_msg.push(0x01u8); // JOIN type
                                        new_join_msg.extend_from_slice(user_id.as_bytes()); // 16 bytes
                                        new_join_msg.extend_from_slice(&timestamp.to_le_bytes()); // 8 bytes
                                        new_join_msg.extend_from_slice(&username_len.to_le_bytes()); // 2 bytes
                                        new_join_msg.extend_from_slice(username_bytes); // variable

                                        // Replace the original message with the enhanced version
                                        msg = Message::Binary(new_join_msg);
                                    }
                                    ...
                                    .await
                                    {
                                        ...
                                        .await
                                        {
                                            Ok(participants) => {
                                                ...
                                                if let Some(room_connections) =
                                                    state.collaboration_rooms.get(&room_uuid)
                                                {
                                                    let join_response_msg =
                                                        Message::Binary(response_data);
                                                    for conn_ref in room_connections.iter() {
                                                        let conn_id = conn_ref.key();
                                                        let sender = conn_ref.value();
                                                        if sender
                                                            .send(join_response_msg.clone())
                                                            .is_err()
                                                        {
                                    ...
                                    history.retain(|stored_msg| {
                                        if let Message::Binary(stored_data) = stored_msg {
                                            ...
                                            if stored_msg_type < 0x10 {
                                                ...
                                                return true; // Keep other non-snapshot server messages
                                            }
                                    ...
                                    if chat_user == user_id {
                                        ...
                                        if data.len() >= 27 + msg_length {
                                                ...
                                                std::str::from_utf8(&data[27..27 + msg_length])
                                            {
                                                ...
                                                // Reconstruct CHAT message with login name
                                                ...
                                                new_chat_msg.extend_from_slice(user_id.as_bytes()); // 16 bytes
                                                new_chat_msg
                                                    .extend_from_slice(&timestamp.to_le_bytes()); // 8 bytes
                                                new_chat_msg
                                                    .extend_from_slice(&username_len.to_le_bytes()); // 2 bytes
                                                new_chat_msg.extend_from_slice(username_bytes); // variable
                                                new_chat_msg
                                                    .extend_from_slice(&msg_len.to_le_bytes()); // 2 bytes
                                                new_chat_msg
                                                    .extend_from_slice(chat_text.as_bytes()); // variable

                                                // Replace the original message with the enhanced version
                                                msg = Message::Binary(new_chat_msg);
                                            }
                                        }
                                    }
                                }
                            ...
                            if is_owner && data.len() >= 19 {
                                if let Ok(sender_uuid) = bytes_to_uuid(&data[1..17]) {
                                    if sender_uuid == user_id {
                                        let url_length =
                                            u16::from_le_bytes([data[17], data[18]]) as usize;

                                        if data.len() >= 19 + url_length {
                                            if let Ok(post_url) =
                                                std::str::from_utf8(&data[19..19 + url_length])
                                            {
                                                info!(
        ...
        let (history_count, history_bytes) = {
            ...
            if should_store {
                ...
                // Update activity cache on any storable message
                state.last_activity_cache.insert(room_uuid, Instant::now());
            }
    ...
    if let Some(room) = state.collaboration_rooms.get(&room_uuid) {
        if room.len() > 1 {
            // Only send if there are other participants
            // Create LEAVE message: [0x09][UUID:16][timestamp:8][usernameLength:2][username:variable]
            let mut leave_msg = vec![0x09u8]; // LEAVE
            leave_msg.extend_from_slice(user_id.as_bytes()); // 16 bytes

            let timestamp = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_millis() as u64;
            leave_msg.extend_from_slice(&timestamp.to_le_bytes()); // 8 bytes

            let username_bytes = user_login_name.as_bytes();
            let username_len = username_bytes.len() as u16;
            leave_msg.extend_from_slice(&username_len.to_le_bytes()); // 2 bytes
            leave_msg.extend_from_slice(username_bytes); // variable
...
async fn handle_save_request(
    ...
    // Calculate paint_duration from session start to now (when saving)
    let now = chrono::Utc::now();
    let created_at_utc = session.created_at.and_utc();
    let duration = now - created_at_utc;

    // Calculate the remainder microseconds after accounting for full days
