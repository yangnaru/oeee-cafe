...
export function attachWindowDrag(
  ...
  const onPointerMove = (event: PointerEvent) => {
    ...
    options.onPosition({
      x: Math.max(
        0,
        Math.min(event.clientX - offset.x, window.innerWidth - rect.width),
      ),
      y: Math.max(
        options.minimumY ?? 0,
        Math.min(event.clientY - offset.y, window.innerHeight - rect.height),
      ),
    });
  };
 ...
 * position it should move to, or null when it is already reachable.
 */
export function clampWindowPosition(
  ...
  return {
    ...
    y: Math.max(minimumY, Math.min(position.y, window.innerHeight - rect.height)),
  };
}
